...
public class EventPaymentAdminHandler implements StepHandler {
    ...
    public StepResult handle(UpdateContext ctx, Session session) {
        ...
        String regId = approve
                ? cb.substring("pay_approve_".length())
                : cb.substring("pay_reject_".length());

        if (approve) {
            Long userChatId = null;

            // Try to mark paid via backend and get chatId
            try {
                String url = backendUrl + "/events/v1/registrations/" + regId + "/mark-paid";
                ...
                if (resp.statusCode() == 200) {
                    ...
                    if (chatIdNode != null && !chatIdNode.isNull()) {
                        userChatId = chatIdNode.asLong();
                    }
                }
            } catch (Exception e) {
                log.warnf("Failed to call mark-paid for reg=%s: %s", regId, e.getMessage());
            }

            // Fallback: look up from pending store
            var stored = pendingStore.getUserChatId(regId);
            if (stored.isPresent()) {
                userChatId = stored.get();
            }
