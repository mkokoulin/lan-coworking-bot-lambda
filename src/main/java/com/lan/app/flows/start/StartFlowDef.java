package com.lan.app.flows.start;

public final class StartFlowDef {
    
    public static final String FLOW = "start";
    public static final String STEP_SHOW           = "start:show";
    public static final String STEP_PROFILE        = "start:profile";
    public static final String STEP_LOGOUT         = "start:logout";
    public static final String STEP_LOGIN          = "start:login";
    public static final String STEP_LOGIN_PHONE    = "start:login_phone";
    public static final String STEP_DEDUCT_CONFIRM = "start:deduct_confirm";
    public static final String STEP_DEDUCT_DO      = "start:deduct_do";
    public static final String STEP_DONE           = "start:done";

    private StartFlowDef() {}
}
