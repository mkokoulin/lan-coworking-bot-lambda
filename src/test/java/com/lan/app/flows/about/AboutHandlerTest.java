...
class AboutHandlerTest {
    ...
    private static UpdateContext ctx() {
        return new UpdateContext(100L, "private", 200L, null, "/about", null, null, false, "bob", null, null, null);
    }

    @Test
    void handle_sendsPhotoAndTextAndStaysOnSend() {
        Session s = session();

        StepResult result = handler.handle(ctx(), s);

        assertThat(result).isEqualTo(StepResult.stay(AboutFlowDef.FLOW, AboutFlowDef.STEP_SEND));
        verify(telegramClient).sendPhoto(eq(100L), anyString(), eq(""));
