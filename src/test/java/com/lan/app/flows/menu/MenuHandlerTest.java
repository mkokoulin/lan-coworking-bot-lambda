...
class MenuHandlerTest {
    ...
    private static UpdateContext ctx() {
        return new UpdateContext(100L, "private", 200L, null, "/menu", null, null, false, "bob", null, null, null);
    }

    @Test
    void handle_sendsMenuTextAndStaysOnShow() {
        Session s = session();

        StepResult result = handler.handle(ctx(), s);

        assertThat(result).isEqualTo(StepResult.stay(MenuFlowDef.FLOW, MenuFlowDef.STEP_SHOW));
